...
export const departRoute = new Elysia({ prefix: "/departs" })
    ...
    .get(
        "/all",
        async ({ status, query }) => {
            const orgDeparts = await DepartService.getOrgDeparts({ orgId: query.orgId });
        ...
        },
        {
            async beforeHandle({ user, query }) {
                ...
                if (!org) return status("Forbidden", { error: "Un-registered org", orgId });
                if (org.ownerId !== user.id) return status("Forbidden", { error: "Access Denied!" });
            },
